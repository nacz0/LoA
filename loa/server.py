...
def run_server(config: AppConfig, runtime: AgentRuntime | None = None) -> None:
    runtime = runtime or AgentRuntime(config)
    handler = make_handler(config, runtime)
...
def make_handler(config: AppConfig, runtime: AgentRuntime) -> type[BaseHTTPRequestHandler]:
    ...
    class LoARequestHandler(BaseHTTPRequestHandler):
                ...
                self._send_json(
                    {
                        ...
                        "providers": runtime.provider_names(),
                    }
                )
                return
        ...
        def _handle_openai_chat(self) -> None:
            ...
            try:
                result = runtime.chat_messages(agent_name=model, messages=messages)
            except (AgentError, ProviderError) as exc:
                self._send_error(HTTPStatus.BAD_GATEWAY, str(exc))
                return
